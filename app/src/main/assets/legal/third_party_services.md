...
| **Pi coding agent** | Minimal terminal coding agent installed and run on-device inside the Alpine PRoot environment | You, via your own provider API keys configured inside `pi` via the terminal | The `pi` process talks directly to whatever provider you configured | Inside `pi`'s own configuration / auth files inside the PRoot environment | No | [Pi license / terms](https://github.com/earendil-works/pi) | Delete the local runtime or uninstall via Settings | Independent third-party project; not affiliated with AndCode |
| **GitHub** | Optional: create/browse pull requests, "Star on GitHub" prompt, `gh` CLI inside the Linux runtime | You, via GitHub's OAuth device-flow login | The app calls the GitHub API directly with your token; the in-runtime `gh` CLI does the same | Encrypted in Android preferences, plus the local runtime environment and `.git-credentials` while Git integration is enabled | No | [GitHub Terms of Service](https://docs.github.com/site-policy/github-terms/github-terms-of-service) | **Disconnect** in **Settings → GitHub**; delete the local runtime for immediate file removal | Independent; not affiliated with AndCode |
...
| **Firebase Analytics** (Google) | Opt-in anonymous app-open/session and runtime outcome metrics | N/A (app-level Firebase project, no end-user login) | The Firebase SDK sends telemetry directly to Firebase; AndCode excludes prompts, code, responses, tokens, and workspace paths | N/A (no user credential; app-level Firebase config) | This *is* a Google-operated collection service, not an AndCode server | [Firebase Terms of Service](https://firebase.google.com/terms) / [Google Privacy Policy](https://policies.google.com/privacy) | Disabled by default; enable or withdraw consent in Settings | Independent Google service; not affiliated with the AI-agent integrations above |
