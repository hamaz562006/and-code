...
| **Piコーディングエージェント** | 端末内のAlpine PRoot環境にインストール・実行される最小限のターミナルコーディングエージェント | 利用者本人。ターミナル経由で`pi`内に設定したプロバイダーAPIキーによる | `pi`プロセスが設定されたプロバイダーへ直接通信 | PRoot環境内の`pi`自身の設定／認証ファイル内 | いいえ | [Piの規約／ライセンス](https://github.com/earendil-works/pi) | ローカルランタイムの削除、または設定画面からアンインストール | 独立した第三者プロジェクト。AndCodeとは提携していません |
