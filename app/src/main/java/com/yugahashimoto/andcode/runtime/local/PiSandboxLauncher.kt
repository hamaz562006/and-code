...
object PiSandboxLauncher {
    ...
    fun command(runtime: LocalRuntimeInstaller.InstalledRuntime, workspaceHostDir: String, workingDirectory: String, arguments: List<String>): List<String> =
        buildList {
            add(runtime.commandSuite.proot.absolutePath)
            add("--kill-on-exit"); add("--link2symlink"); add("-0"); add("-r"); add(runtime.rootfs.absolutePath)
            add("-b"); add("/dev"); add("-b"); add("/proc"); add("-b"); add("/sys"); add("-b"); add("/system")
            add("-b"); add("$workspaceHostDir:/workspace")
            addAll(DeviceStorage.bindArguments())
            add("-w"); add(workingDirectory)
            add(PI_BINARY); addAll(arguments)
        }

    fun environment(runtime: LocalRuntimeInstaller.InstalledRuntime, tmp: File, githubToken: String? = null): Map<String, String> =
